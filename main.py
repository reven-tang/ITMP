...
# from hosts.utils.nmap_discovery_server import NmapNet
# from hosts.utils.nmap_discovery_server import NmapDocker
# from hosts.utils.nmap_discovery_server import NmapKVM
# from hosts.utils.nmap_discovery_server import NmapVMX
from hosts.utils.nmap_discovery_server import snmp_begin
# from hosts.utils.j_filter import FilterRules
# from hosts.utils.get_pv_relation import GetHostType
# from hosts.utils.machines import Machines
# from hosts.utils.nmap_discovery_server import NetDevLogin
...
def main():
    ...
    :return:
    '''
    with open('hosts/conf/scanhosts.yaml') as f:
        yml_conf = f.read()
    s_conf = yaml.load(yml_conf)
    ...
    #             HostsInfo.objects.update_or_create(ip=item,hostname=is_net[0],sn=is_net[1],mathine_type="Network device")
    #         else:
    #             HostsInfo.objects.update_or_create(ip=item,ssh_port=key_not_login_list[item][0],ssh_status=0)
    ...
    #         is_net = nm.query(item)
    #         if is_net[0] or is_net[1]:
    #             HostsInfo.objects.update_or_create(ip=item,hostname=is_net,mathine_type="Network device")
    #         else:
    #             HostsInfo.objects.update_or_create(ip=item,ssh_status=0)
    #             other_sn = item.replace('.','')
    #             ob = OtherMachineInfo.objects.filter(sn_key=other_sn)
    #             if not ob:
