# -*- coding:utf-8 -*-
#########################################################################
# File Name: nmap_discovery_server.py
...
class J_ssh_do(object):
    ...
    def non_root_rsa_do(self,login_info,cmd_list=""):
        try:
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ...
    def run(self,ip,cmd):
        if ip and cmd:
            print(".......................ip",ip)
            ip_item = HostsInfo.objects.get(ip=ip)
            cn = prpcrypt()
            ...
            if ip_item.ssh_type==3:
                ...
                res = self.non_root_rsa_do(login_info,cmd)
            return res
        else:
            return ""
